...
Local issue files from `issues/` are provided at the start of context. Parse them to understand the open issues.
...
1. **Critical bugfixes**
2. **Development infrastructure** — getting tests, types, and dev scripts ready is an important precursor to building features.
3. **Tracer bullets for new features** — small slices of functionality that go through all layers of the system, allowing you to test and validate your approach early. This helps identify potential issues and ensures the overall architecture is sound before investing significant time in development. TL;DR: build a tiny end-to-end slice of the feature first, then expand it out.
